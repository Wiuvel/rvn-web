...
export async function GET(
    ...
    try {
      ...
      `)
        ...
        const formattedComments: CommentResponse[] = (comments || []).map((c: Record<string, unknown>) => {
            const usersRel = c.users;
            const authorData = Array.isArray(usersRel) ? usersRel[0] : usersRel;
            const a = authorData as { id: string; username: string; user_id: string; avatar?: string | null } | null;
            return {
                id: c.id as string,
