...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tickets || tickets.length === 0) {
      return setCorsHeaders(NextResponse.json({ tickets: [] }));
    }
    ...
    if (ticketIds.length > 0 && supabaseAdmin) {
      try {
        ...
        if (rpcError) {
          ...
          let messagesWithSenderType: Array<{
            id: string;
            sender_id: string;
            sender_type?: string | null;
          }> | null = null;
          ...
          const messagesNeedingAttachments = lastMessages.filter(
            (msg: RpcLastMessage) => !msg.message_text,
          );
    ...
    return setCorsHeaders(NextResponse.json({ tickets: ticketsWithLastMessage }));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (subject.trim().length > TICKET_SUBJECT_MAX_LENGTH) {
      return setCorsHeaders(NextResponse.json({ error: ERROR_SUBJECT_TOO_LONG }, { status: 400 }));
    }
    ...
    if (message.length > MESSAGE_MAX_LENGTH) {
      return setCorsHeaders(NextResponse.json({ error: ERROR_MESSAGE_TOO_LONG }, { status: 400 }));
    }
    ...
    if (ticketCount >= MAX_TICKETS_PER_USER) {
      return setCorsHeaders(
        NextResponse.json({ error: ERROR_MAXIMUM_TICKET_LIMIT_REACHED }, { status: 400 }),
      );
    }
