...
export async function POST(
  ...
  try {
    ...
    if (sessionId && csrfToken) {
      ...
      return setCorsHeaders(
        NextResponse.json({ error: 'Invalid request. Please refresh the page.' }, { status: 403 }),
      );
    }
    ...
    if (!isSupport && ticket.status === 'closed') {
      return setCorsHeaders(
        NextResponse.json({ error: ERROR_CANNOT_SEND_TO_CLOSED_TICKET }, { status: 400 }),
      );
    }
    ...
    if (attachments && Array.isArray(attachments) && attachments.length > 0) {
      const attachmentRecords = attachments.map(
        (att: {
          storagePath: string;
          storageUrl: string;
          fileName: string;
    ...
    if (!isSupport) {
      ...
      if (!hasSupportMessage) {
        ...
        if (!existingSystemMessage || existingSystemMessage.length === 0) {
          const { error: autoMessageError } = await supabaseAdmin.from('support_messages').insert({
            ticket_id: ticketId,
    ...
    return setCorsHeaders(NextResponse.json({ message: newMessage, success: true }));
  } catch (error) {
