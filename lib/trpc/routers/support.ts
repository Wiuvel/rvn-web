...
export const supportRouter = router({
  ...
  tickets: router({
    ...
    /** Пагинация сообщений для useInfiniteQuery (cursor = offset) */
    getMessages: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const nextCursor =
          (messages || []).length === limit ? cursor + (messages || []).length : null;

        return { messages: messagesWithSenderType, nextCursor };
      }),
