...
describe('avatar-gradients', () => {
  describe('getAvatarUrl', () => {
    ...
    it('returns null for numeric avatar id (gradient)', () => {
      ...
      expect(getAvatarUrl('5')).toBe(null);
    });
    ...
    it('encodes path parts', () => {
      const url = getAvatarUrl('s3:avatars/user-id/1770569737475.jpg');
      expect(url).toContain('/images/users/');
      expect(url).toBe('/images/users/user-id/1770569737475.jpg');
    });
  ...
  describe('getGradientClasses', () => {
    it('returns gradient class for numeric id', () => {
      ...
      expect(c).toContain('from-blue-500');
    });
    ...
    it('uses fallback for invalid id', () => {
      const c = getGradientClasses('99');
      expect(c).toContain('bg-gradient-to-r');
    });
  });
  ...
  describe('getBannerUrl', () => {
    it('returns null for non-s3', () => {
      ...
      expect(getBannerUrl('0')).toBe(null);
    });
