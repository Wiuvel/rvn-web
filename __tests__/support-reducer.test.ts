...
describe('supportReducer', () => {
  it('should handle SET_USER_DATA', () => {
    ...
    const newState = supportReducer(initialState, {
      type: 'SET_USER_DATA',
      payload: userData,
    });
  ...
  it('should handle UPDATE_ACTIVE_TICKET_MESSAGES', () => {
    ...
    state = supportReducer(state, {
      type: 'UPDATE_ACTIVE_TICKET_MESSAGES',
      payload: messages,
    });
  ...
  it('should handle TOGGLE_USER_MENU', () => {
    // Default is false
    let newState = supportReducer(initialState, {
      type: 'TOGGLE_USER_MENU',
    });
    expect(newState.userMenuOpen).toBe(true);

    newState = supportReducer(newState, {
      type: 'TOGGLE_USER_MENU',
    });
  ...
  it('should handle SHOW_NOTIFICATION', () => {
    ...
    expect(newState.notification.show).toBe(true);
    expect(newState.notification.message).toBe('Error occurred');
    expect(newState.notification.type).toBe('error');
  });
  ...
  it('should handle HIDE_NOTIFICATION', () => {
    ...
    // Message should be preserved or cleared? Implementation says preserved but show=false
