...
Real-time коммуникация обрабатывается **отдельным WebSocket сервером** (`rvn-socket-server`), независимым от веб-приложения (Vinext, Next.js API-совместимый). Веб-приложение подключается к нему как клиент и отправляет broadcast-команды через внутренний REST API.
...
2. Socket.IO передает auth-токен в handshake
...
4. Все подключенные клиенты в комнате получают событие
