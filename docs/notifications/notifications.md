...
Система уведомлений информирует пользователей о событиях в тикетах поддержки: ответы от поддержки, изменения статуса обращений. Уведомления доставляются в реальном времени через WebSocket и доступны через API с cursor-based пагинацией. Источник событий — [Система поддержки](../support/support.md).
