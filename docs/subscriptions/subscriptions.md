...
Пока панель Remnawave не настроена — или пока не зарегистрирован ни один администратор — система подписок **неактивна**, а не падает с ошибкой. `getSubscriptionSystemStatus()` (`lib/integrations/remnawave.ts`) сообщает:
...
- `subscription.servers` и `subscription.publicServers` возвращают `[]`, а не бросают исключение — нет спама `Internal server error on subscription.servers` в логах при каждом заходе на дашборд.
