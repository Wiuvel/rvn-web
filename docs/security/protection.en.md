...
`proxy.ts` is the only middleware in the project. It uses the Next.js middleware API (compatible with Vinext, which exposes the same edge-style `proxy.ts` convention) and dispatches requests in this fixed order:
...
| `X-Content-Type-Options` | `nosniff` (set by `next.config.ts:headers()`, honored by Vinext). |
| `X-Frame-Options` | `DENY` (set by `next.config.ts:headers()`, honored by Vinext). |
| `Referrer-Policy` | `strict-origin-when-cross-origin` (set by `next.config.ts:headers()`, honored by Vinext). |
